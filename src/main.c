...
void verify_click(global_t *all)
{
    ...
    if (mouse.x >= BUTTON[3].pos.x && mouse.x <= BUTTON[3].pos.x + 120 && mouse.y >= BUTTON[3].pos.y && mouse.y <= BUTTON[3].pos.y + 60) {
        if (EVENT.type == sfEvtMouseButtonPressed) {
            all->clicked_file = !all->clicked_file;
        }
    }
    if (mouse.x >= BUTTON[4].pos.x && mouse.x <= BUTTON[4].pos.x + 120 && mouse.y >= BUTTON[4].pos.y && mouse.y <= BUTTON[4].pos.y + 60) {
        if (EVENT.type == sfEvtMouseButtonPressed) {
            all->clicked_edit = !all->clicked_edit;
        }
    }
    if (mouse.x >= BUTTON[5].pos.x && mouse.x <= BUTTON[5].pos.x + 120 && mouse.y >= BUTTON[5].pos.y && mouse.y <= BUTTON[5].pos.y + 60) {
        if (EVENT.type == sfEvtMouseButtonPressed) {
...
void clickable_section(global_t *all)
{
    ...
    mouse.y >= BUTTON[7].pos.y && mouse.y <= BUTTON[7].pos.y + 60 && all->clicked_file == true) {
        if (EVENT.type == sfEvtMouseButtonPressed) {
            clear_canvas(all);
        }
    }
    if (mouse.x >= BUTTON[8].pos.x && mouse.x <= BUTTON[8].pos.x + 120 &&
    mouse.y >= BUTTON[8].pos.y && mouse.y <= BUTTON[8].pos.y + 60 && all->clicked_file == true) {
        if (EVENT.type == sfEvtMouseButtonPressed) {
            save_canvas(all);
        }
    }
    if (mouse.x >= BUTTON[10].pos.x && mouse.x <= BUTTON[10].pos.x + 120 &&
    mouse.y >= BUTTON[10].pos.y && mouse.y <= BUTTON[10].pos.y + 60 && all->clicked_file == true) {
        if (EVENT.type == sfEvtMouseButtonPressed) {
            printf("open file\n");
        }
    }
    if (mouse.x >= BUTTON[10].pos.x && mouse.x <= BUTTON[10].pos.x + 120 &&
    mouse.y >= BUTTON[10].pos.y && mouse.y <= BUTTON[10].pos.y + 60 && all->clicked_edit == true) {
        if (EVENT.type == sfEvtMouseButtonPressed) {
            all->pen_selected = true;
        }
    }
    if (mouse.x >= BUTTON[11].pos.x && mouse.x <= BUTTON[11].pos.x + 120 &&
    mouse.y >= BUTTON[11].pos.y && mouse.y <= BUTTON[11].pos.y + 60 && all->clicked_edit == true) {
        if (EVENT.type == sfEvtMouseButtonPressed) {
            all->pen_selected = false;
        }
    }
    if (mouse.x >= BUTTON[12].pos.x && mouse.x <= BUTTON[12].pos.x + 120 &&
    mouse.y >= BUTTON[12].pos.y && mouse.y <= BUTTON[12].pos.y + 60 && all->clicked_help == true) {
        if (EVENT.type == sfEvtMouseButtonPressed) {
            system("zenity --info --text='Login: Rares'");
        }
    }
    if (mouse.x >= BUTTON[13].pos.x && mouse.x <= BUTTON[13].pos.x + 120 &&
    mouse.y >= BUTTON[13].pos.y && mouse.y <= BUTTON[13].pos.y + 60 && all->clicked_help == true) {
        if (EVENT.type == sfEvtMouseButtonPressed) {
